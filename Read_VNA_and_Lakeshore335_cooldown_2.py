...
Instrument = RsInstrument(resource, True, False)
"""
...
date = "20251229-Tlbco-500nm-3"

count = 0

templist = np.arange(5, 95, 0.5);
# templist = [9]
...
for temp in templist:
    ...
    success, range_changed = temp_reader.set_temperature(temp,wait = True,heater_range = heater_range0)
    
    if range_changed == 1:
        
        heater_range0 = heater_range0 + 1
    ...
    pc_filename = folder + '\\YBCO_resonator_-30dBm_%.3fK_%d.s2p'  %(current_temp,count)                                              # Name and path of the s2p file on the PC
