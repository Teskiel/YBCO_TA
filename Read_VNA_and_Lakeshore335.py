...
def comprep():
    ...
    Instrument.opc_timeout = 40000                                   # Timeout for opc-synchronised operations
...
while count<count_max:
    ...
    folder = r'D:\YBCO\VNAMeas\data\%s\-25dBm'%(date)
    ...
    folder_data = os.path.join(folder, '4K-0mW-laser')
    ...
    # pc_filename = folder_data + '\\tlbco_in_cryostat201_small_3_6GHz_light_%d_%.3fK.s2p'%(count,current_temp)                                              # Name and path of the s2p file on the PC

    pc_filename = folder_data + '\\tlbco_in_cryostat201_small_3_6GHz_light_%d_%.3fK.s2p'%(count,current_temp)
